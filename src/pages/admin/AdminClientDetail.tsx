...
const planColors: Record<string, string> = {
  starter: 'border-white/[0.08] text-slate-400',
...
const statusColors: Record<string, string> = {
  ...
  inactive: 'border-white/[0.08] text-slate-500',
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-body">{label}</label>
      {children}
    </div>
  )
}
...
function TextInput({ value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <input
      ...
      className="input-dark w-full px-3.5 py-2.5 text-sm font-body"
    />
  )
}
  ...
  return (
    <div style={CARD_STYLE} className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <p className="font-display font-bold text-slate-200 text-[13px] uppercase tracking-wider">{title}</p>
        {extra}
      </div>
      {children}
    </div>
...
export default function AdminClientDetail() {
  ...
  if (loading) return (
    <div className="flex justify-center py-20">
      <div className="w-6 h-6 border-2 border-white/[0.08] border-t-zinc-900 rounded-full animate-spin" />
    </div>
  )
  ...
  return (
    <div className="space-y-6 pb-8 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/admin/clients')} className="p-2 rounded-xl text-slate-600 hover:bg-white/[0.05] hover:text-slate-300 transition-colors">
        ...
        <div>
          ...
          {!isNew && org.created_at && (
            <p className="text-xs text-slate-600 font-medium mt-0.5">Criado em {formatDate(org.created_at)}</p>
          )}
        </div>
        {!isNew && (
      ...
      {!isNew && (
        <div className="flex gap-1 p-1 bg-white/[0.05] rounded-2xl w-fit">
          {([
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                activeTab === tab.key
                  ? 'bg-[#0c0e1a] text-slate-100 shadow-sm'
        ...
        <>
            ...
            <div className="space-y-6">
                    ...
                    {plans.map(p => (
                      <button key={p} onClick={() => handlePlanChange(p)}
                        className={`px-3.5 py-1.5 rounded-xl text-xs font-black border-2 transition-all ${
                          org.plan === p ? `${planColors[p]} bg-[#0c0e1a] shadow-sm` : 'border-white/[0.08] text-slate-600 hover:border-white/[0.15]'
                        }`}
                      >
                        {planLabel(p)}<span className="ml-1.5 text-[10px] text-slate-600">R${PLAN_PRICES[p]}</span>
                      </button>
                    ))}
                  </div>
                </Field>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {statuses.map(s => (
                        <button key={s} onClick={() => setOrg(o => ({ ...o, status: s }))}
                          className={`px-3.5 py-1.5 rounded-xl text-xs font-black border-2 transition-all ${
                            org.status === s ? `${statusColors[s]} bg-[#0c0e1a] shadow-sm` : 'border-white/[0.08] text-slate-600 hover:border-white/[0.15]'
                          }`}
                        >
                          {statusLabel(s)}
                        </button>
                    ...
                    <div className="flex gap-2">
                      {(['formal', 'friendly'] as const).map(t => (
                        <button key={t} onClick={() => setOrg(o => ({ ...o, agent_tone: t }))}
                          className={`px-3.5 py-1.5 rounded-xl text-xs font-black border-2 transition-all ${
                            org.agent_tone === t ? 'border-emerald-500 text-emerald-300 bg-[#0c0e1a] shadow-sm' : 'border-white/[0.08] text-slate-600 hover:border-white/[0.15]'
                          }`}
                        >
              ...
              {isNew && (
                <Card title="Acesso ao Dashboard">
                  ...
                  <p className="text-xs text-slate-600">O usuário pode alterar a senha após o primeiro acesso.</p>
                </Card>
              )}

              {/* Redefinir senha (edição) */}
              {!isNew && (
                <div className="bg-[#0c0e1a] rounded-[1.125rem] border border-white/[0.06] shadow-sm p-6 space-y-4">
                  <div className="flex items-center gap-2">
                    <KeyRound className="w-4 h-4 text-slate-600" />
                    <p className="font-black text-slate-100 text-sm uppercase tracking-wider">Redefinir Senha</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      ...
                      className="px-3.5 py-2.5 rounded-xl border border-white/[0.08] text-sm bg-[#0c0e1a] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                    />
                    <input
                      type="password"
                      value={newPass}
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                  >
              ...
              <Card
                ...
                extra={org.evolution_instance ? (
                  <button onClick={checkEvolutionConnection} disabled={checkingConn}
                    className="flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-200 transition-colors disabled:opacity-50">
          ...
          {(settingUp || setupResult) && (
            <div className="bg-[#0c0e1a] rounded-[1.125rem] border border-white/[0.06] shadow-sm p-6 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <p className="font-black text-slate-100 text-sm uppercase tracking-wider">
              ...
              {setupResult && (
                <div className="space-y-2">
                  {setupResult.steps.map(step => (
                    <div key={step.id} className="flex items-start gap-3 p-3 rounded-xl bg-[#0d0f1c]">
                      {step.ok
                        ? <CheckCircle2 className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />
                        : <XCircle className="w-4 h-4 text-slate-600 mt-0.5 shrink-0" />}
                      <div className="min-w-0">
                        <p className={`text-xs font-black ${step.ok ? 'text-slate-200' : 'text-slate-500'}`}>{step.label}</p>
                        <p className="text-[11px] text-slate-600 mt-0.5">{step.detail}</p>
                      </div>
                    </div>
                  ))}
                  {setupResult.webhookUrl && (
                    <p className="text-[11px] text-slate-600 pt-1 font-mono break-all">Webhook: {setupResult.webhookUrl}</p>
                  )}
                  {!isNew && org.evolution_instance && (
                    <button onClick={() => runSetup(id!)} disabled={settingUp}
                      className="flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-200 mt-2 transition-colors disabled:opacity-50">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleSave} disabled={saving || settingUp}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 disabled:opacity-60 transition-colors">
            ...
            {!isNew && org.evolution_instance && !setupResult && (
              <button onClick={() => runSetup(id!)} disabled={settingUp}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/[0.08] text-slate-400 text-sm font-bold hover:bg-[#0d0f1c] disabled:opacity-50 transition-colors">
      ...
      {!isNew && activeTab === 'agente' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-6">
              <Card title="Perfil do Agente">
                  ...
                  <div className="flex gap-2">
                    {(['friendly', 'formal'] as const).map(t => (
                      <button key={t} onClick={() => setAgentTone(t)}
                        className={`px-3.5 py-1.5 rounded-xl text-xs font-black border-2 transition-all ${
                          agentTone === t ? 'border-emerald-500 text-emerald-300 bg-[#0c0e1a] shadow-sm' : 'border-white/[0.08] text-slate-600 hover:border-white/[0.15]'
                        }`}
                      >
                ...
                <Field label="Mensagem de Saudação">
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-white/[0.08] text-sm bg-[#0c0e1a] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 resize-none"
                  />
                </Field>
              </Card>
              ...
              <Card title="Instruções Personalizadas">
                ...
                <textarea
                  ...
                  rows={8}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-white/[0.08] text-sm bg-[#0c0e1a] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 resize-none font-mono"
                />
              </Card>
            </div>
            ...
            <div className="space-y-6">
              {/* Serviços */}
              <div className="bg-[#0c0e1a] rounded-[1.125rem] border border-white/[0.06] shadow-sm overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between px-6 pt-6 pb-4">
                  <div>
                    <p className="font-black text-slate-100 text-sm uppercase tracking-wider">Serviços</p>
                    <p className="text-xs text-slate-600 mt-0.5">PDF enviado automaticamente ao confirmar agendamento</p>
                  </div>
                  <button
                    onClick={() => { setShowAddService(v => !v); setNewService({ name: '', description: '', price: '' }) }}
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 transition-colors shrink-0"
                  >
                ...
                {/* Formulário inline */}
                {showAddService && (
                  <div className="mx-6 mb-4 rounded-2xl border border-white/[0.08] bg-[#0d0f1c] p-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-black text-slate-600 uppercase tracking-wider mb-1.5">Nome *</label>
                        <input
                          type="text"
                          ...
                          className="w-full px-3 py-2.5 rounded-xl border border-white/[0.08] text-sm bg-[#0c0e1a] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                          autoFocus
                        />
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-600 font-bold pointer-events-none">R$</span>
                          <input
                            type="text"
                            value={newService.price}
                            onChange={e => setNewService(s => ({ ...s, price: e.target.value }))}
                            placeholder="150,00"
                            className="w-full pl-8 pr-3 py-2.5 rounded-xl border border-white/[0.08] text-sm bg-[#0c0e1a] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                          />
                        </div>
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-600 uppercase tracking-wider mb-1.5">Descrição</label>
                      <input
                        type="text"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 disabled:opacity-40 transition-colors"
                      >
                        <Plus className="w-3.5 h-3.5" />
                        Adicionar
                      </button>
                      <button
                        ...
                        className="px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:text-slate-400 transition-colors"
                      >
                        Cancelar
                      </button>
                    </div>
                ...
                {services.length > 0 ? (
                      ...
                      return (
                        <div key={svc.id}>
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-4 px-6 py-4 hover:bg-white/[0.03]/80 transition-colors text-left"
                          >
                            <div className="flex-1 min-w-0 flex items-center gap-3">
                              <span className="text-sm font-bold text-slate-100 truncate">{svc.name}</span>
                              {svc.price && (
                          ...
                          {/* Painel expandido */}
                          {isOpen && (
                            <div className="px-6 pb-5 pt-1 bg-zinc-50/60 border-t border-white/[0.06]">
                              {svc.description && (
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-white/[0.08] bg-[#0c0e1a] text-xs font-bold text-slate-300 hover:border-white/[0.15] hover:bg-[#0d0f1c] transition-colors disabled:opacity-50"
                                >
                                ...
                                {svc.pdf_url && (
                                  <>
                                    <span className="text-xs text-slate-600 truncate max-w-[160px]">{svc.pdf_name}</span>
                                    <button onClick={() => removePdf(svc.id)} className="p-1.5 rounded-lg text-slate-700 hover:text-red-400 transition-colors">
                  ...
                  <div className="text-center py-10 px-6 mb-2">
                    ...
                    <p className="text-xs text-slate-600 mt-1">Adicione os serviços oferecidos pela clínica</p>
                  </div>
                )}
