...
// ── Large score circle (expanded) ───────────────────────────────────────────
function ScoreCircleLg({ score, rank }: { score: number; rank: number }) {
  const size   = 80
  const stroke = 6
  const r      = (size - stroke * 2) / 2
  const circ   = 2 * Math.PI * r
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-xl font-black text-slate-800 leading-none">{score.toFixed(1)}</span>
      </div>
      <div className="absolute -top-1 -left-1 w-5 h-5 rounded-full bg-[#2C82B5] text-white flex items-center justify-center text-[9px] font-black">
        #{rank}
      </div>
    </div>
  )
              ...
              >
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      {ar?.yearsExperience && ar.yearsExperience !== '-' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[10px] font-bold border border-slate-200">
                          ...
                          {ar.yearsExperience.toUpperCase()}
                        </span>
                      )}
                      {ar?.city && ar.city !== '-' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[10px] font-bold border border-slate-200">
                ...
                {isOpen && (
                  <div className={cn(
                    ...
                    {/* Top: big circle + name + status */}
                    <div className="flex items-center gap-5">
                      <ScoreCircleLg score={score} rank={rank} />
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          {ar?.yearsExperience && ar.yearsExperience !== '-' && (
                            <span className="inline-flex items-center gap-1 text-xs text-slate-500">
                              <Briefcase className="w-3 h-3" /> {ar.yearsExperience}
                            </span>
                          )}
                          {ar?.city && ar.city !== '-' && (
                            <span className="inline-flex items-center gap-1 text-xs text-slate-500">
                              <MapPin className="w-3 h-3" /> {ar.city}
                              {ar.neighborhood && ar.neighborhood !== '-' ? `, ${ar.neighborhood}` : ''}
                            </span>
                          )}
                          {c.candidate_phone && (
                            <span className="inline-flex items-center gap-1 text-xs text-slate-500">
                              <Phone className="w-3 h-3" /> {c.candidate_phone}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    {/* Contacts */}
                    {(c.candidate_phone || (ar?.phoneNumbers?.length ?? 0) > 0) && (
                      <div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-slate-400 inline-block" />
                          Contatos
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[c.candidate_phone, ...(ar?.phoneNumbers ?? [])].filter(Boolean)
                            .filter((v, i, a) => a.indexOf(v) === i)
                            .map((phone, i) => (
                              <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-xl border border-slate-200 text-xs text-slate-600 font-medium">
